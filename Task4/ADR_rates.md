...
1. **Источник данных — Excel.**  
   На этапе MVP ставки хранятся в XLS/XLSX-файлах. Сохраняется риск ошибок ручного ввода, несогласованности форматов и дублирования строк.
2. **Отсутствие прямой интеграции с АБС.**  
   Решение не синхронизирует ставки с ядром банка в реальном времени; актуальность гарантируется только через утверждённые файлы бэк-офиса.
3. **Партнёрский кол-центр не поддерживает API/SFTP.**  
   Для обмена используется корпоративная почта. Это накладывает ограничения по скорости доставки и контролю статусов.
4. **Ограниченная асинхронность в MVP.**  
   Kafka не внедрена; обмен событиями отсутствует. Обновления ставок зависят от расписания выгрузок.
5. **Регламентное обновление ставок.**  
   В MVP данные обновляются вручную после загрузки Excel. Автоматическое обновление из внешних источников пока не предусмотрено.

---

#### ⚠️ **Риски**

| Категория            | Описание                                                                                              | Меры снижения                                                                    |
|----------------------|-------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| **Бизнес-риски**     | Ошибки или задержки в загрузке ставок могут привести к неправильным консультациям клиентов и жалобам. | Автоматическая валидация, история версий и ручная активация после проверки.      |
| **Операционные**     | Потеря или дублирование писем с выгрузкой партнёру.                                                   | Ретрай-механизм, контрольная сумма/подпись, отчёты о доставке, резервные адреса. |
| **Безопасность**     | Утечка файла ставок через почту или отправка не на тот адрес.                                         | DLP-проверки, белые списки доменов, шифрование TLS, контроль получателей.        |
| **Технические**      | Рост нагрузки на REST API при увеличении числа потребителей (КЦ, сайт, ИБ).                           | Кэширование, горизонтальное масштабирование, переход на Kafka в целевом решении. |
| **Эксплуатационные** | Недостаток DevOps-экспертизы для сопровождения нового сервиса.                                        | CI/CD, централизованный мониторинг и дашборды, документация по деплою.           |
| **Проектные**        | Несогласованность расписания обновлений между банком и партнёром.                                     | Утверждённый SLA                                                                 |
